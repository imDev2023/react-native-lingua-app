...
export default function SignUpScreen() {
  ...
  const handleSignUp = async () => {
    ...
    if (sendError) {
      setFormError(sendError.message ?? "Could not send a verification code.");
      return;
    }
  ...
  const handleResend = async () => {
    ...
    if (error) {
      throw new Error(error.message ?? "Could not resend the code.");
    }
  };
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: "#ffffff" }}>
        ...
        >
          <View className="flex-1 px-6 pt-2 pb-8">
            ...
            {/* Heading */}
            <Text className="h1 mt-5">Create your account</Text>
